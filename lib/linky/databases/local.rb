    ...
    class Local < Resource
      ...
      def connection
        ...
        if do_init
          ...
          @dbh.do("CREATE TABLE sessions (id INTEGER PRIMARY KEY, query TEXT, status TEXT, total INTEGER, done INTEGER, first_id TEXT, last_id TEXT, label_length INTEGER, value_length INTEGER, exception BLOB)")
        end
        @dbh
      end
    end
